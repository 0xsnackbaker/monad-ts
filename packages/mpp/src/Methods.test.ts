...
import { accounts, makeChallenge, token } from "./helpers.test.js";
...
describe("Methods", () => {
  describe("charge", () => {
    ...
    test("creates a challenge from the method", () => {
    ...
    test("supports chainId in methodDetails", () => {
      const challenge = Challenge.fromMethod(Methods.charge, {
        realm: "test.example.com",
        id: "test-id",
        request: {
      ...
      expect(
        (challenge.request["methodDetails"] as Record<string, unknown>)?.[
          "chainId"
        ],
      ).toBe(10143);
    });

    test("omits methodDetails when no chainId", () => {
      const challenge = Challenge.fromMethod(Methods.charge, {
        realm: "test.example.com",
        id: "test-id",
        request: {
    ...
    test("serializes and deserializes challenge", () => {
